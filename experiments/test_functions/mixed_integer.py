...
class MixedIntegerCOCO(object):
    ...
    def __init__(self, random_seed=None, problem_id=None):
        ...
        self.suggested_init = sample_mixed_init_points(self.problem.lower_bounds, self.problem.upper_bounds, 
                                                            self.num_discrete, n_points=20, random_seed=random_seed).float()
    ...
    def generate_random_points(self, n_points, random_seed=None):
        return sample_mixed_init_points(self.problem.lower_bounds, self.problem.upper_bounds, 
                                                            self.num_discrete, n_points=n_points, random_seed=self.random_seed if random_seed is None else random_seed).float()
