...
def sample_speed_reducer_points(n_points, random_seed=None):
...
def sample_weld_points(n_points, random_seed=None):
    if random_seed is not None:
        rng_state = torch.get_rng_state()
        torch.manual_seed(random_seed)
    init_points = []
    for _ in range(n_points):
        ...
        random_point.append(torch.FloatTensor(1).uniform_(0.0625, 2))
        random_point.append(torch.FloatTensor(1).uniform_(0, 10))
        random_point.append(torch.FloatTensor(1).uniform_(2, 10))
        random_point.append(torch.FloatTensor(1).uniform_(0.0625, 2))
        init_points.append(random_point)
    return torch.tensor(init_points).float()


def sample_mixed_init_points(lbs, ubs, num_discrete, n_points, random_seed=None):
    if random_seed is not None:
        rng_state = torch.get_rng_state()
        torch.manual_seed(random_seed)
    init_points = []
