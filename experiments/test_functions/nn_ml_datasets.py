...
class Problem(object):
    ...
    def __call__(self, x):
        ...
        kwag_dict = {'hidden_layer_sizes': hidden_layer_sizes[int(x[0])], 'activation': activation[int(x[1])], 'batch_size': batch_size[int(x[2])], 'learning_rate': learning_rate[int(x[3])],
                'early_stopping': early_stopping[int(x[4])], 'learning_rate_init': x[5], 'momentum':x[6], 'alpha':x[7], 'solver':'sgd'}
        print(kwag_dict)
...
class NN_ML_Datasets(object):
    def __init__(self, random_seed=None, problem_id=None):
        ...
        self.problem = Problem(dataset=self.problem_id, dimension=self.dimension, num_discrete=self.num_discrete, num_continuous=self.num_continuous, lower_bounds=self.lower_bounds, upper_bounds=self.upper_bounds)
