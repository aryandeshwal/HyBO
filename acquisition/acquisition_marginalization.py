...
def acquisition_expectation(x, inference_samples, partition_samples, n_vertices, acquisition_func=expected_improvement,
    ...
    for s in range(len(inference_samples)):
        ...
        grouped_x = group_input(x[:, :inference_samples[s].model.kernel.num_discrete], sorted_partition=partition_samples[s], n_vertices=n_vertices)
...
def inference_sampling(input_data, output_data, n_vertices, hyper_samples, log_order_variance_samples, log_beta_samples, lengthscales_samples, partition_samples,
    ...
    for s in range(len(hyper_samples)):
        ...
        kernel = MixedDiffusionKernel(log_order_variances = log_order_variance_samples[s], grouped_log_beta=grouped_log_beta, fourier_freq_list=freq_samples[s], 
                            fourier_basis_list=basis_samples[s], lengthscales=lengthscales_samples[s],
        ...
        grouped_input_data = group_input(input_data=input_data[:, :model.kernel.num_discrete], sorted_partition=partition_samples[s], n_vertices=n_vertices)# need to understand this?
...
def prediction_statistic(x, inference_samples, partition_samples, n_vertices):
    ...
    var_sample_list = []
    for s in range(len(inference_samples)):
        grouped_x = group_input(input_data=x[:, :inference_samples[s].model.kernel.num_discrete], sorted_partition=partition_samples[s], n_vertices=n_vertices)
        grouped_x = torch.cat((grouped_x, x[:, inference_samples[s].model.kernel.num_discrete:]), dim=1)
