...
def greedy_ascent(x_init, inference_samples, partition_samples, edge_mat_samples, n_vertices,
    ...
    while n_ascent < max_n_ascent:
        x_nbds = neighbors(x[:inference_samples[0].model.kernel.num_discrete], partition_samples, edge_mat_samples, n_vertices, uniquely=True)
        x_nbds = torch.cat((x_nbds, x[inference_samples[0].model.kernel.num_discrete:].unsqueeze(0).repeat(x_nbds.size(0), 1)), dim=1)
