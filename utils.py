...
def displaying_and_logging(
    ...
    for i in range(eval_inputs.size(0)):
        ...
        time_str = (
            time.strftime("%H:%M:%S", time.gmtime(time_list[i]))
            + "("
        ...
        torch.save(
            {
                ...
                "log_beta_samples": log_beta_samples,
                "lengthscale_samples": lengthscale_samples,
                "lov": log_order_var_samples,
            },
            pklfilename,
        )
